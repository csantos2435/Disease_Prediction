  ...
  return getDatabasesPath().then(
    (dbPath) {
      ...
      return openDatabase(
        path,
        onCreate: (db, version) {
          db.execute('CREATE TABLE diseaseprediction('
...
// POST
Future<int> save(DiseasePrediction diseaseprediction) {
...
// GET
Future<List<DiseasePrediction>> findAll() {
  return createDatabase().then(
    (db) {
  ...
  return createDatabase().then(
    (db) {
      ...
      // usuarioMap['id'] = usuario.id;
        usuarioMap['user'] = usuario.user ;
        usuarioMap['cpfUser'] = usuario.cpfUser ;
        usuarioMap['email'] = usuario.email ;
        usuarioMap['funcao'] = usuario.funcao ;
        usuarioMap['password'] = usuario.password ;
        usuarioMap['passwordChecks'] = usuario.passwordChecks ;
